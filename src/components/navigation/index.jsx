...
const Navigation = () => {
  ...
  const isMobile = size < 480; // Assuming 480px as the breakpoint for mobile
      ...
      {isMobile ? (
        <>
          ...
          >
            {BtnList.slice(0, BtnList.length / 2).map((btn) => {
              return <NavButton key={btn.label} x={0} y={0} {...btn} variants={item} />;
            })}
          </motion.div>
        ...
        >
          {BtnList.map((btn, index) => {
            ...
            return <NavButton key={btn.label} x={x} y={y} {...btn} variants={item} />;
          })}
        </motion.div>
      )}
    </div>
